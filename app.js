...
const grammarRules = {
  ...
  presentContinuous: {
    ...
    generator: function() {
      ...
      const tmpl = templates[Math.floor(Math.random() * templates.length)];
      return { jp: jpText, en: enText, template: tmpl };
    }
  },
  beVerb: {
    ...
    generator: function() {
      ...
      const tmpl = templates[Math.floor(Math.random() * templates.length)];
      return { sentence, answer: chosen.answer, template: tmpl };
    }
  },
  will: {
    type: "fill-in-the-blank",
    generator: function() {
      ...
      const tmpl = templates[Math.floor(Math.random() * templates.length)];
      return { sentence, answer: "will", template: tmpl };
    }
  },
  past: {
    type: "math",
  ...
  comparative: {
    ...
    generator: function() {
      ...
      const tmpl = templates[Math.floor(Math.random() * templates.length)];
      return { sentence, answer: adj.comparative, template: tmpl };
    }
  },
  questionFormation: {
    type: "writing",
  ...
  passiveVoice: {
    ...
    generator: function() {
      ...
      const verb = verbs[Math.floor(Math.random() * verbs.length)];
      const obj = objects[Math.floor(Math.random() * objects.length)];
      const activeSentence = `${subj} ${verb.active} ${obj.text}.`;
      const passiveSentence = `${obj.text.charAt(0).toUpperCase() + obj.text.slice(1)} ${verb.passive} by ${subj.toLowerCase()}.`;
      const templates = [
        'Change the following sentence into passive voice: "{sentence}"',
        '次の文を受動態に書き換えよ："{sentence}"'
      ];
      const tmpl = templates[Math.floor(Math.random() * templates.length)];
      return { sentence: activeSentence, answer: passiveSentence, template: tmpl };
    }
  },
  listening: {
    type: "listening",
  ...
  maxPremium: {
    type: "speaking",
    generator: function() {
      return { en: "Discuss the impact of social media on global communication.", jp: "ソーシャルメディアが世界のコミュニケーションに与える影響について議論してください。", template: "Premium: {jp}" };
    }
  }
};
...
function generateQuestion(topic) {
  ...
  if (rule.type === "listening" || rule.type === "speaking") {
    return { type: rule.type, question: questionText, answer: correctAnswer };
  }
  
  if (topic === "bonus") {
    return { type: "bonus", question: questionText, answer: correctAnswer, hint: dataItem.hint || (correctAnswer.charAt(0) + "_".repeat(correctAnswer.length - 1)) };
  }
  ...
  if (qType === "multiple" && rule.generator && rule.type !== "writing") {
    ...
    return { type: "multiple", question: questionText, options: options, answer: correctAnswer };
  } else {
    return { type: "writing", question: questionText, answer: correctAnswer };
  }
}
...
// ───────── demolingoMAX Free Trial 機能 ─────────
function isMaxTrialActive() {
  const trialStart = localStorage.getItem("maxTrialStart");
  if (!trialStart) return false;
  const trialTime = new Date(trialStart);
  const now = new Date();
  return (now - trialTime) < 48 * 60 * 60 * 1000;
}
function unlockMaxTrial() {
  localStorage.setItem("maxTrialStart", new Date().toISOString());
  alert("demolingoMAX Free Trialが開始されました！48時間無料でプレミアムコンテンツをお楽しみください。");
  showStages();
}
...
const stages = [
  ...
  {
    name: "Stage 6: demolingoMAX Premium",
    lessons: [
      { name: "Premium Lesson", topic: "maxPremium" }
    ]
  }
];
...
function showStages() {
  ...
  // demolingoMAX Unlockボタン（300コイン以上かつ未解放の場合のみ表示）
  const maxUnlockBtn = document.getElementById("max-unlock-btn");
  if (getCoins() >= 300 && !localStorage.getItem("maxTrialStart")) {
    maxUnlockBtn.style.display = "block";
    maxUnlockBtn.innerText = "Unlock demolingoMAX Free Trial (300コイン必要)";
    maxUnlockBtn.onclick = unlockMaxTrial;
  } else {
    maxUnlockBtn.style.display = "none";
  }
  ...
  stages.forEach((stage, index) => {
    ...
    if (stage.name === "Stage 6: demolingoMAX Premium") {
      if (localStorage.getItem("maxTrialStart") && isMaxTrialActive()) {
        stageDiv.style.opacity = "1";
        stageDiv.onclick = () => showLessons(index);
      } else {
        stageDiv.style.opacity = "0.4";
        stageDiv.onclick = () => alert("demolingoMAX Free Trialの期間は終了しました。有料コンテンツのご利用をご検討ください。");
      }
    } else {
      stageDiv.onclick = () => showLessons(index);
    }
...
function startLesson(lesson) {
  if (lesson.topic === "maxPremium" && (!localStorage.getItem("maxTrialStart") || !isMaxTrialActive())) {
    alert("demolingoMAX Free Trialの期間は終了しています。有料プランが必要です。");
    return;
  }
  ...
  if (lesson.topic !== "maxPremium") { addCoins(5); }
  updateProgress();
  loadQuestion();
}
